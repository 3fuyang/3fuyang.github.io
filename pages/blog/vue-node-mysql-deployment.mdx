...
8. **Nginx 配置与启动：**
   ...
   9. 进入宝塔面板——软件商店—— Nginx 设置——配置修改，在 **http** 块（**注意**：不是 **server** 块）内增加以下**反向代理**代码：
   ...
   10. ``` js
         server{
          ...
          listen 8090;
          // ECS 服务器外网IP地址
          server_name localhost 106.15.78.201;
          location / {
              // 前端目录
   ...
   12. 在浏览器中访问 “http://ip:port"，即可进入前端页面
       ...
       3. ``` js
          server{
              listen 8090; # 前端页面端口
              server_name localhost 106.15.78.201; # 服务器 IP
              ...
              ## 配置后端反向代理
              location ~ /api/ {
                  proxy_pass http://106.15.78.201:8081/; # 后端运行地址
              }
          }
      ```
