---
title: [翻译] Promise/A+ 规范
date: 2022-10-03T20:56:00.000+00:00
lang: zh
duration: 4min
path: /blog/promise-a-plus
---
...
> 1. 根据具体实现的不同，`then`方法返回的 promise 可能与原 promise 完全相等。
> 2. **平台代码**（platform code）是指引擎、环境和 promise 实现代码。实际上，该要求保证`onFulfilled`和`onRejected`在调用`then`后被**异步**执行。这可以通过**宏任务**机制实现（比如`setTimeout`或`setImmediate`），或者**微任务**机制（比如`MutationObserver`或`process.nextTick`）。
...
4. 如果`x`不是函数或对象，则以`x`为值将`promise`转为 fullfied。
